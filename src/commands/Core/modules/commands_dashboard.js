...
export function buildOverviewEmbed(snapshot, guild) { 
  ...
  const categoryLines = snapshot.categories.map((category) => { 
    ...
    const subcommandNote = category.commands.some((c) => c.isSubcommand) ? ' · incluye subcomandos' : ''; 
  ...
  chunks.forEach((chunk, index) => { 
    fields.push({ 
      name: index === 0 ? '📁 Categorías' : '📁 Categorías (cont.)', 
      value: chunk, 
      inline: false, 
    }); 
  }); 
  ...
  fields.push({ 
    name: 'Cómo usar', 
    value: [ 
      '• Selecciona una categoría abajo para administrar comandos y subcomandos', 
      '• `/commands disable` — desactiva una categoría o un comando específico', 
      '• `/commands enable` — vuelve a activar algo', 
    ].join('\n'), 
  }); 

  return createEmbed({ 
    title: '⚙️ Acceso a comandos', 
    description: `Administra los comandos slash y de prefijo de **${guild.name}**. Los subcomandos (por ejemplo, \`birthday list\`) se muestran por separado.`, 
    color: 'info', 
    fields, 
    footer: '🔒 los comandos y configwizard siempre estarán disponibles', 
  }); 
} 
...
export function buildCategoryEmbed(category, guild) { 
  ...
  const statusText = category.categoryDisabled 
    ...
    : category.disabledCount === 0 
      ? 'Todas las entradas habilitadas' 
      : `${category.disabledCount} de ${category.totalCount} deshabilitadas`; 
  ...
  const fields = [ 
    ...
    { 
      name: '📈 Cantidad', 
      value: `${category.enabledCount}/${category.totalCount} habilitadas`, 
      inline: true, 
    }, 
  ]; 
  ...
  chunks.forEach((chunk, index) => { 
    fields.push({ 
      name: index === 0 ? '📋 Comandos y subcomandos' : '📋 (cont.)', 
      value: chunk, 
      inline: false, 
    }); 
  }); 
  ...
  fields.push({ 
    name: 'Cómo usar', 
    value: [ 
      '• Usa el menú desplegable para activar o desactivar comandos o subcomandos individuales', 
      '• **Desactivar todo** desactiva toda la categoría', 
      '• **Limpiar excepciones** vuelve a habilitar las entradas desactivadas individualmente', 
    ].join('\n'), 
  }); 

  return createEmbed({ 
    title: `${category.icon} ${category.displayName}`, 
    description: `Acceso a comandos para **${guild.name}**.`, 
    color: category.categoryDisabled ? 'error' : category.disabledCount > 0 ? 'warning' : 'success', 
    fields, 
    footer: '🔒 Las entradas protegidas no se pueden desactivar', 
  }); 
} 
...
export function buildOverviewComponents(guildId, snapshot) { 
  const categoryOptions = snapshot.categories.slice(0, 25).map((category) => { 
    ...
    return new StringSelectMenuOptionBuilder() 
      ...
      .setDescription(`${status} ${category.enabledCount}/${category.totalCount} habilitadas`.slice(0, 100)) 
...
export function buildCategoryComponents(guildId, category) { 
  ...
  const commandOptions = toggleableCommands.slice(0, 25).map((command) => { 
    ...
    return new StringSelectMenuOptionBuilder() 
      .setLabel(label) 
      .setDescription((enabled ? '🟢 Habilitado — pulsa para desactivar' : '🔴 Deshabilitado — pulsa para habilitar').slice(0, 100)) 
  ...
  const rows = [ 
    new ActionRowBuilder().addComponents( 
      ...
      new ButtonBuilder() 
        ...
        .setLabel(category.categoryDisabled ? 'Habilitar categoría' : 'Deshabilitar categoría') 
    ...
    rows.unshift( 
      new ActionRowBuilder().addComponents( 
        new StringSelectMenuBuilder() 
          ...
          .setPlaceholder('Activa o desactiva un comando o subcomando...') 
...
export async function handleDashboardComponent(interaction, client) { 
  ...
  if (guildId !== interaction.guildId) { 
    return interaction.reply({ 
      content: 'Este panel pertenece a otro servidor.', 
      ephemeral: true, 
    }); 
  } 
  ...
  return interaction.editReply({ content: 'Acción del panel desconocida.', embeds: [], components: [] }); 
} 
